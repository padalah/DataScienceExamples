...
# load omni file
df = spark.read.parquet(f'{blob_url}/parquet_flight_weather_network_joined')
...
counts = rf_input.filter(F.year(rf_input.utc) <= 2017).select('label').groupBy('label').count().toPandas()
display(counts)
...
train = rf_weighted.filter((F.year(rf_weighted.utc) <= 2017))
...
# create test set
test = rf_weighted.filter(F.year(rf_weighted.utc) == 2018)
...
# # define random forest model - baselevel
# rfw2 = RandomForestClassifier(labelCol="label", featuresCol="features", weightCol="weight", numTrees=50)
# pipeline2 = Pipeline(stages=[rfw2])

# # train model
# modelRF = pipeline2.fit(train)
...
# display(predictionsRF.groupby('label', 'prediction').count())

# COMMAND ----------

# # Train a DecisionTree model - basemodel
# dt = DecisionTreeClassifier(labelCol="label", featuresCol="features", maxBins=350)
...
# #Combine base models and build a pipeline
# gen_base_pred_pipeline = Pipeline(stages=[rf, dt])
# gen_base_pred_pipeline_model = gen_base_pred_pipeline.fit(train)

# COMMAND ----------

# baseline_preds = gen_base_pred_pipeline_model.transform(test)

# COMMAND ----------

# display(baseline_preds.groupby('label', 'rf_pred').count())
...
# train the last level classifier 
# # use od-pair, utc to join the rf and dt predictions
# baseline_preds_features = np.transpose(baseline_preds)
# display(baseline_preds_features)

# COMMAND ----------

# # calculate stacking predictions
# lr_model = LogisticRegression(featuresCol='features', labelCol='label', predictionCol='final_prediction', maxIter=100)
# meta_classifier = lr_model.fit(baseline_preds)

# COMMAND ----------

# preds = meta_classifier.transform(test)
